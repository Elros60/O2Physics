...
struct muonQa {
  ...
  template <bool MuonFillMap, bool GlobalMuonFillMap, typename TEventMap, typename TBcs, typename TMuons, typename TMFTTracks, typename TTrack, typename VarC, typename VarT>
  void runDCA(TEventMap const& collisions, TBcs const& bcs, TMuons const& muons, TMFTTracks const& mfts, TTrack const& muon, mch::Track const& mchrealigned, VarC& fgValuesColl, VarT& fgValuesMCH, VarT& fgValuesMCHpv, VarT& fgValuesMFT)
  {
    if constexpr (static_cast<bool>(MuonFillMap)) {

      // track selection
      if (!IsGoodMuon(fgValuesMCH, fgValuesMCHpv, configMuons.fTrackChi2MchUp, 0., 30., configMFTs.fEtaMftLow, configMFTs.fEtaMftUp, configMuons.fRabsLow, configMuons.fRabsUp, configMuons.fSigmaPdcaUp)) {
        return;
      }
  ...
  template <typename TEventMap, typename TBcs, typename TMuons, typename TMFTTracks, typename TMCHTrack, typename TMFTTrack, typename TMuonCls, typename VarC, typename VarT>
  void runResidual(TEventMap const& collisions, TBcs const& bcs, TMuons const& muons, TMFTTracks const& mfts, TMuonCls const& clusters, TMCHTrack const& mchtrack, mch::Track const& mchrealigned, TMFTTrack const& mfttrack, VarC const& fgValuesColl, VarT const& fgValuesMCH, VarT const& fgValuesMCHpv, VarT const& fgValuesMFT)
  {
    if (!IsGoodMuon(fgValuesMCH, fgValuesMCHpv, configMuons.fTrackChi2MchUp, 20., configMuons.fPtMchLow, configMFTs.fEtaMftLow, configMFTs.fEtaMftUp, configMuons.fRabsLow, configMuons.fRabsUp, configMuons.fSigmaPdcaUp)) {
      return;
    }
  ...
  template <typename TEventMap, typename TCandidateMap, typename TBcs, typename TFwdTracks, typename TMFTTracks, typename TMuonCls>
  void runMuonQA(TEventMap const& collisions, TCandidateMap& matchingCandidates, TBcs const& bcs, TFwdTracks const& muons, TMFTTracks const& mfts, TMuonCls const& clusters)
  {
    ...
    for (auto const& muon : muons) {
      ...
      if (static_cast<int>(muon.trackType()) <= 2) { // MFT-MCH-MID(0) or MFT-MCH(2)
        ...
        if (configQAs.fEnableQADCA) {
          runDCA<1, 0>(collisions, bcs, muons, mfts, muon, mchrealigned, fgValuesColl, fgValuesMCH, fgValuesMCHpv, fgValuesMFT);
        }
      }
    ...
    for (auto& [mchIndex, globalMuonsVector] : matchingCandidates) {
      ...
      if (configQAs.fEnableQAResidual) {
        runResidual(collisions, bcs, muons, mfts, clusters, mchtrack, mchrealigned, mfttrack, fgValuesCollGlo, fgValuesMCH, fgValuesMCHpv, fgValuesMFT);
      }

      //// Fill MFT DCA QA checks if required
      if (configQAs.fEnableQADCA) {
        runDCA<0, 1>(collisions, bcs, muons, mfts, nullptr, mch::Track(), fgValuesCollGlo, fgValuesMCH, fgValuesMCHpv, fgValuesMFT);
      }
  ...
  void processMuonQa(MyEvents const& collisions, aod::BCsWithTimestamps const& bcs, MyMuonsWithCov const& muontracks, MyMFTs const& mfttracks, aod::FwdTrkCls const& muonclusters)
  {
    ...
    runMuonQA(collisionSel, matchingCandidates, bcs, muontracks, mfttracks, muonclusters);
